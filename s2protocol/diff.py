...
def diff_things(typeinfo_index, thing_a, thing_b):
    ...
    if type(thing_a) == list or type(thing_a) == tuple:
        if len(thing_a) != len(thing_b):
            print("typeinfo {} diff len: {} {}".format(typeinfo_index, len(thing_a), len(thing_b)))
        else:
        ...
        if type(thing_a) == int:
            if thing_a < 55 or thing_a - 1 != thing_b:
                print("typeinfo {} diff number: {} {}".format(typeinfo_index, thing_a, thing_b))
        else:
            print("typeinfo {} diff string: {} {}".format(typeinfo_index, thing_a, thing_b))
            

def diff(protocol_a_ver, protocol_b_ver):
    print(protocol_a_ver, protocol_b_ver)
